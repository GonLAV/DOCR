  ...
  return (
    <div className="min-h-screen p-6 lg:p-10 space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <AnimatePresence>
          {activeWidgets.includes("documents-overview") && (
            <DashboardWidget
              id="documents-overview"
              title="Documents Overview"
              icon={FileText}
              onRemove={customizeMode ? removeWidget : null}
              isExpanded={expandedWidgets.includes("documents-overview")}
          ...
          {activeWidgets.includes("recent-activity") && (
            <DashboardWidget
              ...
              icon={TrendingUp}
              onRemove={customizeMode ? removeWidget : null}
              isExpanded={expandedWidgets.includes("recent-activity")}
              onToggleExpand={toggleExpand}
            >
              <div className="space-y-3">
                {recentDocs.length > 0 ? recentDocs.slice(0, expandedWidgets.includes("recent-activity") ? 10 : 3).map((doc, index) => (
                  <motion.div
                    key={doc.id}
                    initial={{ opacity: 0, x: -10 }}
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: index * 0.05 }}
                    className="glass rounded-xl p-3 hover:glass-strong transition-all duration-300 group cursor-pointer"
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        <div className={`w-10 h-10 rounded-lg bg-gradient-to-br ${getStatusGradient(doc.status)} flex items-center justify-center shrink-0`}>
          ...
          {activeWidgets.includes("ai-insights") && (
            <DashboardWidget
              ...
              icon={Shield}
              onRemove={customizeMode ? removeWidget : null}
              isExpanded={expandedWidgets.includes("ai-insights")}
              onToggleExpand={toggleExpand}
            >
              <div className="relative w-24 h-24 mx-auto mb-4">
                <svg className="transform -rotate-90 w-24 h-24">
                  ...
                  <circle
                    ...
                    strokeWidth="8"
                    fill="none"
                    strokeDasharray={`${(avgConfidence / 100) * 264} 264`}
                    strokeLinecap="round"
                  />
                  <defs>
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center">
                    <div className="text-2xl font-black text-white">{avgConfidence}</div>
                    <div className="text-[10px] text-cyan-300 font-semibold">TRUST</div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div className="glass rounded-lg p-3 border-l-4 border-rose-500">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-sm font-semibold text-white">High Risk Documents</span>
                  </div>
                  <p className="text-xs text-gray-400">{flagged} documents require review</p>
                </div>
              </div>
            </DashboardWidget>
          )}

          {activeWidgets.includes("performance") && (
            <DashboardWidget
              id="performance"
      ...
      {/* Legacy Main Content - Hidden in customize mode */}
      {!customizeMode && (
        <>
